...
export const ThemeProvider = ({ children }) => {
    ...
    const [theme, setTheme] = useState(() => {
        ...
        return savedTheme || 'dark'; // 'dark', 'light', 'amoled', 'adaptive'
    });

    const [adaptiveColor, setAdaptiveColor] = useState('rgba(0, 0, 0, 0.8)'); // Default dark fallback
    ...
    useEffect(() => {
        ...
        root.classList.remove('theme-dark', 'theme-light', 'theme-amoled', 'theme-adaptive');
        ...
        if (theme === 'light') {
            ...
            // Adaptive uses the extracted video color
            root.style.setProperty('--bg-color', adaptiveColor);
            root.style.setProperty('--text-color', '#ffffff');
            root.style.setProperty('--panel-bg', 'rgba(0, 0, 0, 0.4)');
